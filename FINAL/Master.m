...
IMev = [0.5 0.65 0.8 0.95 1.1 1.25 1.4 1.55 1.7]; 
IMh = 75;%[50 65 80 95 110 125 140 155];

dtv = [50 100 150 200 250 300 350 400 450 500 550 600 650 700 750 800];

sims = 1;
count = 0;
for imi = 1:length(IMev)
...
% Weight = 1;
% 
% t1 = my_random(tot_time(1,:),tr,1000000);
% t2 = my_random(pr1(1,:),tr,1000000);
...
% 
% t1 = my_random(tot_time(2,:),tr,1000000);
% t2 = my_random(pr1(2,:),tr,1000000);
% [y,x] = ecdf(t1+Weight*t2);
...
count = count + 1;
progressbar(count/(length(IMev)*length(dtv)))
    end
end
...
for ii = 1:16
rec11 = reshape(recF_1(1,ii,:),3000,1);
rec22 = reshape(recF_2(1,ii,:),3000,1);
per(ii) = trapz(tff,abs(rec11-rec22)/3000);
end
